...
test('server persists a request and accepts a validated result',{timeout:15000},async t=>{
  ...
  response=await fetch(`http://127.0.0.1:${port}/api/runs/${run.id}/report.html`); assert.equal(response.status,200); assert.match(response.headers.get('content-disposition'),/attachment/); const reportHtml=await response.text(); assert.match(reportHtml,/Opportunity Intelligence/); assert.match(reportHtml,/エグゼクティブ結論/); assert.match(reportHtml,/方法論/);
  response=await fetch(`http://127.0.0.1:${port}/api/runs/${run.id}/report.html?mode=print`); assert.equal(response.status,200); assert.equal(response.headers.get('content-disposition'),'inline'); assert.match(await response.text(),/PDFに保存/);
  ...
  response=await fetch(`http://127.0.0.1:${port}/api/runs/${run.id}/simulation/chat`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({target_id:'a1',question:'なぜこの判断をした？'})}); assert.equal(response.status,201); const chat=await response.json(); assert.equal(chat.message.target_id,'a1'); assert.equal(chat.message.classification,'synthetic_explanation');
});
