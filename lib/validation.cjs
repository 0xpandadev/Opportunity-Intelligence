...
function validateResult(result, expectedRunId) {
  ...
  if (result.market_landscape != null) {
    if (!Array.isArray(result.market_landscape?.entities)) errors.push('market_landscape.entities must be an array');
    const validTargets = {
      megatrend: new Set((Array.isArray(result.megatrends) ? result.megatrends : []).map(item => item?.id).filter(Boolean)),
      whitespace: new Set((Array.isArray(result.whitespaces) ? result.whitespaces : []).map(item => item?.id).filter(Boolean))
    };
    const entityIds = new Set();
    for (const [index, entity] of (result.market_landscape?.entities || []).entries()) {
      if (!entity?.id) errors.push(`market_landscape.entities[${index}].id is required`);
      if (entityIds.has(entity?.id)) errors.push(`duplicate market entity id: ${entity.id}`);
      entityIds.add(entity?.id);
      if (!entity?.name) errors.push(`market_landscape.entities[${index}].name is required`);
      if (!entity?.role) errors.push(`market_landscape.entities[${index}].role is required`);
      if (!Array.isArray(entity?.evidence_ids) || !entity.evidence_ids.length) errors.push(`market_landscape.entities[${index}].evidence_ids must be a non-empty array`);
      if (!Array.isArray(entity?.links) || !entity.links.length) errors.push(`market_landscape.entities[${index}].links must be a non-empty array`);
      if (['listed', 'listed_parent'].includes(entity?.listing?.status)) {
        if (!entity.listing.ticker) errors.push(`market_landscape.entities[${index}] listed entity requires ticker`);
        if (!entity.listing.exchange) errors.push(`market_landscape.entities[${index}] listed entity requires exchange`);
        if (entity.listing.status === 'listed_parent' && !entity.listing.parent) errors.push(`market_landscape.entities[${index}] listed_parent requires parent`);
      }
      for (const [linkIndex, link] of (entity?.links || []).entries()) {
        if (!['megatrend', 'whitespace'].includes(link?.kind)) errors.push(`market_landscape.entities[${index}].links[${linkIndex}].kind is invalid`);
        else if (!validTargets[link.kind].has(link?.id)) errors.push(`market_landscape.entities[${index}].links[${linkIndex}] references unknown ${link.kind}: ${link?.id}`);
        if (!link?.relationship) errors.push(`market_landscape.entities[${index}].links[${linkIndex}].relationship is required`);
      }
    }
  }
