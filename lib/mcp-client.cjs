...
const localManifestPath = path.join(__dirname, '..', 'config', 'mcp-servers.local.json');

function expandPortableValue(value, variables) {
  if (typeof value === 'string') return value.replace(/\$\{([A-Z0-9_]+)\}/g, (match, key) => variables[key] || match);
  if (Array.isArray(value)) return value.map(item => expandPortableValue(item, variables));
  if (value && typeof value === 'object') return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, expandPortableValue(item, variables)]));
  return value;
}

function readManifest() {
  const manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8'));
  const local = fs.existsSync(localManifestPath) ? JSON.parse(fs.readFileSync(localManifestPath, 'utf8')) : { servers:[] };
  const overrides = new Map((local.servers || []).map(server => [server.id, server]));
  const variables = {
    REPO_ROOT:path.join(__dirname, '..'),
    MCP_ROOT:process.env.OI_MCP_ROOT || local.mcp_root || path.resolve(__dirname, '..', '..', 'mcp-servers')
  };
  return expandPortableValue({
    ...manifest,
    servers:(manifest.servers || []).map(server => ({ ...server, ...(overrides.get(server.id) || {}) }))
  }, variables);
}
